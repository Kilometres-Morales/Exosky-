...
teegarden_star_d_coord = SkyCoord(ra=RA_VALUE*u.deg, dec=DEC_VALUE*u.deg, distance=DISTANCE_VALUE*u.pc, obstime=Time("2024-10-01T00:00:00"))

search_radius = 15 * u.deg  # Example search radius around Proxima Centauri b
...
WHERE CONTAINS(
    POINT('ICRS', ra, dec),
    CIRCLE('ICRS', {teegarden_star_d_coord.ra.deg}, {teegarden_star_d_coord.dec.deg}, {search_radius.to(u.deg).value})
)=1
"""
...
proxima_icrs = teegarden_star_d_coord.transform_to(ICRS())
...
plt.title(f"Night Sky from Teegarden's star d (RA: {RA_VALUE}, Dec: {DEC_VALUE})")
plt.legend()
plt.show()