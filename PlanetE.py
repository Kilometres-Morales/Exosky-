...
kepler_22_b_coord = SkyCoord(ra=RA_VALUE*u.deg, dec=DEC_VALUE*u.deg, distance=DISTANCE_VALUE*u.pc, obstime=Time("2024-10-01T00:00:00"))
...
WHERE CONTAINS(
    POINT('ICRS', ra, dec),
    CIRCLE('ICRS', {kepler_22_b_coord.ra.deg}, {kepler_22_b_coord.dec.deg}, {search_radius.to(u.deg).value})
)=1
"""
...
kepler_icrs = kepler_22_b_coord.transform_to(ICRS())

with solar_system_ephemeris.set('jpl'):
    sun_position = get_sun(kepler_icrs.obstime)
...
plt.title(f"Night Sky from Kepler 22b (RA: {RA_VALUE}, Dec: {DEC_VALUE})")
plt.legend()
plt.show()