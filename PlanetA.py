...
#working code that scales according to brightness FOR PROXIMA CENTAURI B
...
RA_VALUE = 217.429  
DEC_VALUE = -62.679  
DISTANCE_VALUE = 1.301  

proxima_centauri_b_coord = SkyCoord(ra=RA_VALUE*u.deg, dec=DEC_VALUE*u.deg, distance=DISTANCE_VALUE*u.pc, obstime=Time("2024-10-01T00:00:00"))
...
WHERE CONTAINS(
    POINT('ICRS', ra, dec),
    CIRCLE('ICRS', {proxima_centauri_b_coord.ra.deg}, {proxima_centauri_b_coord.dec.deg}, {search_radius.to(u.deg).value})
)=1
"""
...
proxima_icrs = proxima_centauri_b_coord.transform_to(ICRS())
...
plt.title(f"Night Sky from Proxima Centauri b (RA: {RA_VALUE}, Dec: {DEC_VALUE})")
plt.legend()
plt.show()